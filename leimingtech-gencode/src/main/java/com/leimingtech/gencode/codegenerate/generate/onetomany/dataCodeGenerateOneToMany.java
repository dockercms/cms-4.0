...
import com.leimingtech.gencode.codegenerate.generate.dataCodeGenerate;
 ...
 */
public class dataCodeGenerateOneToMany implements ICallBack {
	...
	public dataCodeGenerateOneToMany(){
		
	}
	
	public dataCodeGenerateOneToMany(String entityPackage,String entityName,String tableName,List<SubTableEntity> subTabParam,
	...
	public dataCodeGenerateOneToMany(CodeParamEntity codeParamEntity){
	...
	public void generateToFile() {
		...
		dataCodeFactoryOneToMany codeFactoryOneToMany = new dataCodeFactoryOneToMany();
		codeFactoryOneToMany.setCallBack(new dataCodeGenerateOneToMany());
		
		if(createFileProperty.isJspFlag()){
			codeFactoryOneToMany.invoke("lmone2many/dictionary/jspListTemplate.ftl", "ftlList");//一对多[列表页面]
			codeFactoryOneToMany.invoke("lmone2many/dictionary/jspTemplate.ftl", "html");//编辑页面
		}
		if(createFileProperty.isServiceImplFlag()){
			codeFactoryOneToMany.invoke("lmone2many/dictionary/serviceImplTemplate.ftl", "serviceImpl");
		}
		if(createFileProperty.isServiceIFlag()){
			codeFactoryOneToMany.invoke("lmone2many/dictionary/serviceITemplate.ftl", "service");
		}
		if(createFileProperty.isActionFlag()){
			codeFactoryOneToMany.invoke("lmone2many/dictionary/controllerTemplate.ftl", "controller");
		}
		if(createFileProperty.isEntityFlag()){
			//Entity实体生成
			codeFactoryOneToMany.invoke("lmone2many/dictionary/entityTemplate.ftl", "entity");//采用替换方式
		}
		if(createFileProperty.isPageFlag()){
			//Page实体生成
			codeFactoryOneToMany.invoke("lmone2many/dictionary/pageTemplate.ftl", "page");//采用替换方式
		}
	...
	public static void main(String[] args) {
		...
		SubTableEntity po = new SubTableEntity();
		//子表表名
		po.setTableName("lmcms_order_custom");
		//子表生成的实体名
		...
		po2.setTableName("lmcms_order_product");
		...
		codeParamEntityIn.setTableName("lmcms_order_main");
		...
		//一对多代码生成
		dataCodeGenerateOneToMany.oneToManyCreate(subTabParamIn, codeParamEntityIn);
	}
	...
	public static void oneToManyCreate(List<SubTableEntity> subTabParamIn, CodeParamEntity codeParamEntityIn){
		...
		for(SubTableEntity sub:subTabParamIn){
			...
			new dataCodeGenerate(sub.getEntityPackage(),sub.getEntityName(),sub.getTableName(),sub.getFtlDescription(),subFileProperty,StringUtils.isNotBlank(sub.getPrimaryKeyPolicy())?sub.getPrimaryKeyPolicy():"uuid",sub.getSequenceCode(),foreignKeys.toArray(new String[]{}),codeParamEntityIn.getEntityName()).generateToFile();
		}
		
		//[2].主表代码生成
		new dataCodeGenerateOneToMany(codeParamEntityIn).generateToFile();
