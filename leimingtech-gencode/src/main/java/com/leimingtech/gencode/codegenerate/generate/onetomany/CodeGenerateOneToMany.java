...
public class CodeGenerateOneToMany implements ICallBack {
	...
	public CodeGenerateOneToMany(String entityPackage,String entityName,String tableName,List<SubTableEntity> subTabParam,
	...
	public void generateToFile() {
		...
		if(createFileProperty.isJspFlag()){
			codeFactoryOneToMany.invoke("onetomany/jspListTemplate.ftl", "jspList");//一对多[列表页面]
			codeFactoryOneToMany.invoke("onetomany/jspTemplate.ftl", "jsp");//编辑页面
//			if(FTL_MODE_A.equals(ftl_mode)){
//				codeFactoryOneToMany.invoke("jspOnetoMainAddTemplate.ftl", "jspAdd");//一对多[添加页面]
//				codeFactoryOneToMany.invoke("jspOnetoMainEditTemplate.ftl", "jspEdit");//一对多[编辑页面]
//			}else if(FTL_MODE_B.equals(ftl_mode)){
//				codeFactoryOneToMany.invoke("jspOnetoMainAddTemplate_tab.ftl", "jspAdd");//一对多[添加页面]
//				codeFactoryOneToMany.invoke("jspOnetoMainEditTemplate_tab.ftl", "jspEdit");//一对多[编辑页面]
//			}
		}
		...
		if(createFileProperty.isEntityFlag()){
			//Entity实体生成
			codeFactoryOneToMany.invoke("onetomany/entityTemplate.ftl", "entity");//采用替换方式
		}
		if(createFileProperty.isPageFlag()){
			//Page实体生成
			codeFactoryOneToMany.invoke("onetomany/pageTemplate.ftl", "page");//采用替换方式
		}
	...
	public static void oneToManyCreate(List<SubTableEntity> subTabParamIn,CodeParamEntity codeParamEntityIn){
		System.out.println("----lmcms----Code-----Generation-----[一对多数据模型："+codeParamEntityIn.getTableName()+"]------- 生成中。。。");
		//[1].子表代码生成
		...
		for(SubTableEntity sub:subTabParamIn){
			...
			new CodeGenerate(sub.getEntityPackage(),sub.getEntityName(),sub.getTableName(),sub.getFtlDescription(),subFileProperty,StringUtils.isNotBlank(sub.getPrimaryKeyPolicy())?sub.getPrimaryKeyPolicy():"uuid",sub.getSequenceCode(),foreignKeys.toArray(new String[]{})).generateToFile();
		}
		
		//[2].主表代码生成
		new CodeGenerateOneToMany(codeParamEntityIn).generateToFile();
		System.out.println("----lmcms----Code----Generation------[一对多数据模型："+codeParamEntityIn.getTableName()+"]------ 生成完成。。。");
	}
