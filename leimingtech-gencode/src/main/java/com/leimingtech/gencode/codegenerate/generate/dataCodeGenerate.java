 ...
 */
public class dataCodeGenerate implements ICallBack {
	...
	public dataCodeGenerate(){
		
	}
	
	public dataCodeGenerate(String entityPackage,String entityName,String tableName,String ftlDescription,CreateFileProperty createFileProperty
	...
	public dataCodeGenerate(String entityPackage,String entityName,String tableName,String ftlDescription,CreateFileProperty createFileProperty
			,String primaryKeyPolicy,String sequenceCode,String[] foreignKeys,String parentEntityName){
		this.entityName = entityName;
		this.entityPackage = entityPackage;
		this.tableName = tableName;
		this.ftlDescription = ftlDescription;
	...
	public void generateToFile() {
		System.out.println("----lmcms---Code----Generation----[子表模型:"+tableName+"]------- 生成中。。。");

		CodeFactory codeFactory = new CodeFactory();
		codeFactory.setCallBack(new dataCodeGenerate());

		codeFactory.invoke("lmone2many/dictionary/jspListSubTemplate.ftl", "ftlList");//详细页面-Table风格
		codeFactory.invoke("lmone2many/dictionary/jspSubTemplate.ftl", "html");//详细页面-Table风格
		if(createFileProperty.isActionFlag()){
			codeFactory.invoke("lmone2many/dictionary/controllerSubTemplate.ftl", "controller");
		}
		if(createFileProperty.isEntityFlag()){
			//Entity实体生成
			codeFactory.invoke("lmone2many/dictionary/entityTemplate.ftl", "entity");
		}
		if(createFileProperty.isServiceImplFlag()){
			codeFactory.invoke("lmone2many/dictionary/serviceImplSubTemplate.ftl", "serviceImpl");
		}
		if(createFileProperty.isServiceIFlag()){
			codeFactory.invoke("lmone2many/dictionary/serviceISubTemplate.ftl", "service");
		}
		System.out.println("----lmcms----Code----Generation-----[子表模型："+tableName+"]------ 生成完成。。。");
	}
	...
	public static void main(String[] args) {
		...
		new dataCodeGenerate().generateToFile();
