...
import ${bussiPackage}.service.${sub.entityPackage}.${sub.entityName}ServiceI;
</#list>
...
public class ${entityName}ServiceImpl implements ${entityName}ServiceI {
	...
	<#list subTab as sub>
	/** ${sub.ftlDescription}接口 */
	@Autowired
	private ${sub.entityName}ServiceI ${sub.entityName?uncap_first}Service;
	</#list>
	...
	public void delete(${entityName}Entity ${entityName?uncap_first}) {
		...
		<#list subTab as sub>
		List<${sub.entityName}Entity> ${sub.entityName?uncap_first}List = ${sub.entityName?uncap_first}Service.getListByPid(${entityName?uncap_first}.getId());
		if (${sub.entityName?uncap_first}List != null && ${sub.entityName?uncap_first}List.size() > 0) {
			...
			for (int i = ${sub.entityName?uncap_first}MaxIndex; i >= 0; i--) {
				${sub.entityName?uncap_first}Service.delete(${sub.entityName?uncap_first}List.get(i));
			}
		}
		</#list>
