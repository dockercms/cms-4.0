...
import com.leimingtech.cms.json.DataGrid;
...
public class ${entityName}Controller extends BaseController {
	...
	public void datagrid(${entityName}Entity ${entityName?uncap_first},HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		//查询条件组装器
		com.leimingtech.platform.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, ${entityName?uncap_first});
		try{
		//自定义追加查询条件
		<#list columns as po>
	...
	public ModelAndView ${sub.entityName?uncap_first}List(${entityName}Entity ${entityName?uncap_first}, HttpServletRequest req) {
		...
		<#list sub.foreignKeys as key>
		    <#if key?lower_case?index_of("${lmcms_table_id}")!=-1>
		Object ${lmcms_table_id}${sub_index} = ${entityName?uncap_first}.get${lmcms_table_id?cap_first}();
		    <#else>
	    ...
	    try{
	    	List<${sub.entityName}Entity> ${sub.entityName?uncap_first}EntityList = systemService.findHql(hql${sub_index},<#list sub.foreignKeys as key><#if key?lower_case?index_of("${lmcms_table_id}")!=-1>${lmcms_table_id}${sub_index}<#else>${key?uncap_first}${sub_index}</#if><#if key_has_next>,</#if></#list>);
