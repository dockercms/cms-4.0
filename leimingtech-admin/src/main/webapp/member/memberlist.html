     ...
     <tbody>
     	<#list memberList as member>
	        <tr>
	          ...
	          <td class="t1">${member.id}</td>
	          <td class="t1">${member.realname}</td>
	          <td class="t2">${member.email}</td>
	          <td class="t3">${member.birthday}</td>
	          <td class="t4">${member.sex}</td>
	          <td class="t5"><a href="javascript:;" onclick="modify(${member.id})">修改</a><a href="javascript:;" onclick="del(${member.id})">删除</a></td>
	        </tr>
        </#list>
     </tbody>
   </table>
...
<script>
	...
	function modify(id){
		$.ajax({
			type:'post',
			url:'registerController.do?modify',
			data:'id='+id,
			dataType:'text',
			success:function(msg){
				$('#rightcontent').empty();
	...
	*/
	...
	if(pageIndex>count){
		...
		$.ajax({
			type:'post',
			url:'registerController.do?registerList',
