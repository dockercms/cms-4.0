...
|	Version: 1.0				|
|	Author:	韦向阳				|
|	Email: wxy008@163.com		|
|	完成于：2010年08月13日			|
|	http://hi.baidu.com/韦向阳	|
|--------------------------------
*/
//AutoComplete Object
...
/*        if ((this.messages[i].length >= this.message.length) && ((this.messages[i].substring(0, this.message.length)) == this.message)) {
            this.likemsgs[j++] = this.messages[i];
				...
				var messages_s=this.messages[i];
			for(var p=0;p<messages_s.length;p++){
				var a=messages_s.charAt(p);
				//	alert(a);
				//	alert(this.messages);   charAt
				if(this.message==a){
					this.likemsgs[j++] = messages_s;	
				}
			}
