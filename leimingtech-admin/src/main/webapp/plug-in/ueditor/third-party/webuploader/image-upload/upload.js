...
function setState( val ) {
    ...
    switch ( state ) {
        ...
        case 'finish':
            ...
            if ( stats.successNum ) {
                //alert( '上传成功' );
            	//alert(uploader.getFiles()[0].name);
            	uploaderGroupSuccess($("#uploaderUrls").val());
            } else {
                // 没有成功的图片，重设
                state = 'done';
                location.reload();
...
uploader.onUploadSuccess=function (file,object){
	$("#uploaderUrls").val($("#uploaderUrls").val()+object.url+",");
};
