...
Eop.Dialog={
	...
	init:function(options){
		...
		var left = "left:"+(document.documentElement.clientWidth-this.opts.width)/2+"px;";
		
		if( $("#dlg_"+self.opts.id).size()==0){
			
	 
			var html ='<div  class="dialog" style="display:none;'+left+'"><div class="dialog_box">';
					html+='<div class="head">';
    ...
    $.fn.gridAjaxPager = function(options) {
    	...
    	function load(pageNo,grid){
    		...
    		$.ajax({
    			url:url,
    			success:function(html){
    				grid.empty().append( $(html).find("#"+grid.attr("id")).children() );
