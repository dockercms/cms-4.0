...
public class TodayInHistoryService {
	...
	private static String extract(String html) {
		...
		if (m.matches()) {
			...
			for (String info : m.group(3).split("  ")) {
				info = info.replace(dateTag, "").replace("（图）", "").replace("&nbsp;&nbsp", "").replace("  ", "").replaceAll("</?[^>]+>", "").trim();
				// 在每行末尾追加2个换行符
				if (!"".equals(info)) {
	...
	public static String getTodayInHistoryInfo() {
		...
		if(result != null){
			...
			StringBuffer sb = new StringBuffer(resultArr[0]);
			int l=0;
			if(resultArr.length>6){
				l = resultArr.length-6;
			}
			for(int i=(resultArr.length-1);i>l;i--){
				//result += resultArr[i];
	...
	public static void main(String[] args) {
		...
		try {
			System.out.print(info.getBytes("UTF-8").length);
