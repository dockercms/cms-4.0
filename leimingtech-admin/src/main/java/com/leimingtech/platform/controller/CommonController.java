...
import com.leimingtech.core.entity.Autocomplete;
...
import com.leimingtech.core.util.JSONHelper;
import com.leimingtech.core.util.MyClassLoader;
import com.leimingtech.core.util.ReflectHelper;
...
import com.leimingtech.platform.service.DeclareService;
...
public class CommonController extends BaseController {
	...
	public ModelAndView openViewFile(HttpServletRequest request) {
		...
		String subclassname = oConvertUtils.getString(request.getParameter("subclassname"), "com.leimingtech.platform.web.system.pojo.base.TSAttachment");
		...
		Object fileobj = declareService.getEntity(fileClass, fileid);
	...
	public void viewFile(HttpServletRequest request, HttpServletResponse response) {
		...
		String subclassname = oConvertUtils.getString(request.getParameter("subclassname"), "com.leimingtech.platform.web.system.pojo.base");
		Class fileClass = MyClassLoader.getClassByScn(subclassname);// 附件的实际类
		Object fileobj = declareService.getEntity(fileClass, fileid);
		ReflectHelper reflectHelper = new ReflectHelper(fileobj);
		...
		declareService.viewOrDownloadFile(uploadFile);
	}
	...
	public void createxml(HttpServletRequest request, HttpServletResponse response) {
		...
		declareService.createXml(importFile);
	}
	...
	public AjaxJson parserXml(HttpServletRequest request, HttpServletResponse response) {
		...
		declareService.parserXml(ctxPath + "/" + fileName);
	...
	public void getAutoList(HttpServletRequest request, HttpServletResponse response, Autocomplete autocomplete) {
		...
		List autoList = declareService.getAutoList(autocomplete);
	...
	public AjaxJson delObjFile(HttpServletRequest request) {
		...
		TSAttachment attachment = declareService.getEntity(TSAttachment.class,fileKey);
		String subclassname = attachment.getSubclassname(); // 子类类名
		Object objfile = declareService.getEntity(MyClassLoader.getClassByScn(subclassname), attachment.getId());// 子类对象
		message = "" + attachment.getAttachmenttitle() + "删除成功";
		declareService.delete(objfile);
	...
	public ModelAndView objfileList(HttpServletRequest request) {
		...
		if (StringUtil.isNotEmpty(busentityName) && StringUtil.isNotEmpty(businessKey)) {
			object = declareService.get(MyClassLoader.getClassByScn(busentityName), businessKey);
