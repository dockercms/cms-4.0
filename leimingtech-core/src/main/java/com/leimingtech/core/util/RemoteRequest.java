...
public class RemoteRequest implements Request {
	...
	public Response execute(String uri, HttpServletResponse httpResponse, HttpServletRequest httpRequest) {
		...
		try {
			...
			// System.out.println( rentity.getContentType().getValue() );
			 response.setContentType(rentity.getContentType().getValue());
	...
	public Response execute(String uri) {
		...
		try {
			...
			eopresponse.setStatusCode(response.getStatusLine().getStatusCode());
			eopresponse.setContentType(entity.getContentType().getValue());
			return eopresponse;
