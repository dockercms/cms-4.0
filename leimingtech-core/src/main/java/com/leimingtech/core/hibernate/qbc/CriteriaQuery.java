...
import com.leimingtech.core.datatable.DataTables;
import com.leimingtech.core.datatable.SortInfo;
...
public class CriteriaQuery implements Serializable {
	...
	public void or(Criterion c1, Criterion... criterions) {

		List<Criterion> list=new ArrayList<Criterion>(Arrays.asList(criterions));

		this.detachedCriteria.add(Restrictions.or(c1, getExpress(list)));
	}
	
	public void or(Criterion c1, List<Criterion> criterions) {
		this.detachedCriteria.add(Restrictions.or(c1, getExpress(criterions)));
	}

	private Criterion getExpress(List<Criterion> criterions) {

		Criterion criterion = criterions.get(criterions.size() - 1);
		criterions.remove(criterions.size() - 1);
		if (criterions.size() > 0) {
			Criterion crierion2 = getExpress(criterions);
			return Restrictions.or(crierion2, criterion);
		}
	 ...
	 * @param keyvalue2
	 */
	public void like(String keyname, String keyvalue, MatchMode matchMode) {
		if (keyvalue != null && keyvalue != "") {
			criterionList.addPara(Restrictions.like(keyname, keyvalue,
					matchMode));
			if (flag) {
