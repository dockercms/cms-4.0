...
public class ContentsController extends ContentsbaseController {
	...
	public String save(ContentsEntity contents, ContentLinkEntity contentLink,
		...
		if(StringUtils.isNotEmpty(funVal0)){
			contentsService.saveOtherContent(contents.getId(), funVal0);
		}
	...
	public ModelAndView contentsTree(HttpServletRequest requset) {
		List<ContentCatVOTreeEntity> list = contentCatService.getSimpleAllBySite(PlatFormUtil.getSessionSiteId());
	...
	public ModelAndView operate(String ids,String toType) {
		Map<String, Object> m = new HashMap<String, Object>();
		SiteEntity site= PlatFormUtil.getSessionSite();
		m.put("siteList", userService.getRoleSite());
		m.put("ids", ids);
		m.put("site", site);
		m.put("toType", toType);
